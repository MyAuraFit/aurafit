...
class DotCarousel(CustomBoxLayout):
...
class DotSpinner(CustomBoxLayout):
    ...
    def get_current_active_dot(self):
        for dot in self.children:
            if dot.active:
                return dot
        return None

    def get_dot_index(self, index: int = 0):
        for dot in self.children:
            if dot.index == index:
                return dot
        return None
    ...
    def deactivate_dots_except(self, dot_instance=None) -> None:
        for dot in self.children:
            if dot == dot_instance:
                continue
            dot.active = False
...
if __name__ == "__main__":
    ...
    class Test(App):
        ...
        def __init__(self, **kwargs):
            ...
            print(self.theme_cls.card_color)
