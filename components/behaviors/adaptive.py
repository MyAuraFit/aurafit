    ...
    def on_adaptive_size(self, _, value: bool) -> None:
        ...
        if isinstance(self, Label):
            if value:
                ...
                self.unbind(texture_size=self.setter_widget_size)
        else:
            if value and not isinstance(self, (FloatLayout, Screen)):
                self.bind(minimum_size=self.setter("size"))
                if not self.children:
                    self.size = (0, 0)
            elif not value and not isinstance(self, (FloatLayout, Screen)):
