...
class CircularProgressIndicator(Widget):
    ...
    .. versionchanged:: 2.0.0
        ...
        Rename `MDSpinner` to `CircularProgressIndicator` class.
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._alpha_anim_in = Animation(
            _alpha=1, duration=0.8, t="out_quad", step=1 / 30
        )
        self._alpha_anim_out = Animation(
            _alpha=0, duration=0.3, t="out_quad", step=1 / 30
        )
    ...
    def on__rotation_angle(self, *args):
        if self._rotation_angle == 0:
            ...
            if not self.determinate:
                _rot_anim = Animation(_rotation_angle=0, duration=2, step=1 / 30)
                _rot_anim.start(self)
        elif self._rotation_angle == 360:
            if self._palette:
                try:
                    Animation(color=next(self._palette), duration=2, step=1 / 30).start(
                        self
                    )
    ...
    def _start_determinate(self, *args):
        ...
        _angle_start_anim = Animation(
            _angle_end=360, duration=self.determinate_time, t="in_out_quad", step=1 / 30
        )
    ...
    def _start_loop(self, *args):
        if self._alpha == 0:
            _rot_anim = Animation(
                _rotation_angle=0, duration=2, t="linear", step=1 / 30
            )
            _rot_anim.start(self)
        ...
        self._alpha_anim_in.start(self)
        _angle_start_anim = Animation(
            _angle_end=self._angle_end + 270,
            duration=0.6,
            t="in_out_cubic",
            step=1 / 30,
        )
    ...
    def _anim_back(self, *args):
        _angle_back_anim = Animation(
            _angle_start=self._angle_end - 8,
            duration=0.6,
            t="in_out_cubic",
            step=1 / 30,
        )
...
class ShimmerProgressIndicator(Widget):
    shimmer_x = NumericProperty(0)
    active = BooleanProperty(False)
    ...
    def start_shimmer(self):
        anim = Animation(
            shimmer_x=self.width * 1.2, duration=1, t="linear", step=1 / 30
        ) + Animation(shimmer_x=0, duration=0, t="linear", step=1 / 30)
        anim.repeat = True
        anim.start(self)
        self.opacity = 1

    def stop_shimmer(self):
        Animation.cancel_all(self, "shimmer_x")
        self.opacity = 0
