...
from kivy.graphics import Mesh, Color, Rectangle, Point, SmoothLine, RenderContext
...
def sigmoid_curve(x_min, x_max, x_range, rate, midpoint):

    # Simulate a sigmoid growth curve
    days = np.linspace(x_min, x_max, x_range)  # 120 days season
    K = 1  # max relative biomass (normalized to 1)
    r = rate  # growth rate
    t0 = midpoint  # midpoint of rapid growth

    # Logistic growth equation
    biomass = K / (1 + np.exp(-r * (days - t0)))
    return np.column_stack((days, biomass))
...
class Graph(Widget):
    ...
    tick_color = ColorProperty([0.25, 0.25, 0.25, 1])
...
class Plot(EventDispatcher):
    ...
    __events__ = ("on_clear_plot", "on_kv_post")
    ...
    def __init__(self, **kwargs):
        ...
        self.bind(color=lambda _, color: setattr(self._gcolor, "rgba", color))
...
class SmoothLinePlot(Plot):
    ...
    def create_drawings(self):
        ...
        with self._grc:
            self._gcolor = Color(*self.color)
            self._gline = SmoothLine(points=[], cap="none", width=self.line_width)
...
if __name__ == "__main__":
    ...
    class TestApp(App):
        ...
        def build(self):
            ...
            x_min, x_max = -50, 50
            y_min, y_max = -1, 1.0
            x0 = (x_min + x_max) / 2  # midpoint
            k = 0.15  # growth rate, tweak as needed

            points = []
            for x in range(x_min, x_max + 1):  # step = 1 day
                logistic = 1 / (1 + math.e ** (-k * (x - x0)))
                y_scaled = y_min + (y_max - y_min) * logistic
                points.append((x, round(y_scaled, 3)))
